...
//Header declaration
//  Vector3d hubo_plus::getCOMofArm(int side);
...
Vector3d hubo_plus::getCOMofArm(int side)
{
    ...
    getArmAngles(side, armAngles);

    for (int i = 0; i < 6; i++) {
        huboArmFK(jointLoc, armAngles, side);
        sumMp += jointLoc * pCOM[i] * mass(i);
        sumMasses += mass(i);
    }

    armCOM = sumMp / sumMasses;
    return armCOM;
}
//Header declaration
//  Vector3d hubo_plus::getCOMofTorso(bool batteryIn=false);

Vector3d hubo_plus::getCOMofTorso()
{
    Vector3d torsoCOM;
